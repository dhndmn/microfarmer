<script>
	...
	const harvestGrams = data.harvests.reduce((total, harvest) => total + harvest.grams, 0);
	...
	function animalMath(harvestGrams) {
		...
		const equivalentAnimals = (harvestGrams / animalGrams).toFixed(1);
